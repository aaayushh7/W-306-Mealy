...
function Home({ user }) {
  ...
  useEffect(() => {
    const setupFCM = async () => {
      try {
        ...
        await axios.post('http://localhost:3000/api/users/fcm-token', 
          { token }, 
  ...
  const markAsAte = async () => {
    try {
      await axios.post('http://localhost:3000/api/users/mark-eaten', {}, {
        headers: {
          Authorization: `Bearer ${await user.getIdToken()}`,
          'Cache-Control': 'no-cache'
        }
  ...
  const reportFoodFinished = async () => {
    try {
      const response = await axios.post('http://localhost:3000/api/report-food-finished', {}, {
        headers: { Authorization: `Bearer ${await user.getIdToken()}` }
      });
